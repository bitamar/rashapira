...
function subtheme_preprocess_node(&$variables) {
  ...
  if ($variables['type'] == 'painting') {
    ...
    if ($image = $wrapper->field_image->value()) {
      ...
      drupal_add_html_head($element, 'subtheme_painting_image');
      ...
      drupal_add_html_head($element, 'subtheme_painting_title');
    }
  }
...
function subtheme_menu_local_task($variables) {
...
function subtheme_menu_item_link($link) {
...
function subtheme_link($variables) {
  if (_subtheme_is_term_active($variables['path'])) {
...
function _subtheme_is_term_active($path) {
